...
public:
  ...
  void FindmMinPathMemoDynamic(bool isVerical);
  void InitMemo();
  void ClearMemo();
  int  GetRowColinMemo(int row,int col) const;
  void PutRowColinMemo(int row,int col, int beste) ;
  /* int  FindmMinPathMemo(int row, int col); */
private:
  ImagePPM image_;
  int height_ = 0;
  int width_ = 0;
